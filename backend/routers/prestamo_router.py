...
def get_prestamos(request: Request, session: Session = Depends(get_db_session), user=Depends(LIST_ALL)):
...
def get_prestamos_activos(request: Request, session: Session = Depends(get_db_session), user=Depends(LIST_ALL)):
    prestamos = PrestamoService(session).get_activos()
    return etag_response(request, prestamos)


@router.get("/vencidos", response_model=list[PrestamoResponse])
def get_prestamos_vencidos(request: Request, session: Session = Depends(get_db_session), user=Depends(LIST_ALL)):
...
def create_prestamo(
    body: PrestamoCreate,
    session: Session = Depends(get_db_session),
    user: SessionUser = Depends(require_permission("Prestamo", "create")),
):
    return PrestamoService(session).create(
