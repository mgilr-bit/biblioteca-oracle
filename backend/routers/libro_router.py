...
threadpool automáticamente, que es lo correcto dado que `get_session()`
...
def get_generos(request: Request, user=Depends(READ)):
...
def libros_bajo_stock(request: Request, user=Depends(READ)):
    with get_session() as session:
...
def export_libros_csv(user=Depends(require_permission("Libro", "read"))):
    with get_session() as session:
...
def get_estadisticas(request: Request, user=Depends(READ)):
    with get_session() as session:
...
def create_libro(body: LibroCreate, user=Depends(require_permission("Libro", "create"))):
    with get_session() as session:
        result = LibroService(session).create(body.model_dump())
    return result


@router.get("/{id_libro}", response_model=LibroResponse)
def get_libro(id_libro: int, request: Request, user=Depends(READ)):
    with get_session() as session:
...
def update_libro(id_libro: int, body: LibroUpdate, user=Depends(WRITE)):
    with get_session() as session:
        result = LibroService(session).update(id_libro, body.model_dump())
    return result


@router.patch("/{id_libro}/copias", response_model=MessageResponse)
def update_copias(id_libro: int, body: LibroCopiasUpdate, user=Depends(WRITE)):
    with get_session() as session:
        result = LibroService(session).update_copias(id_libro, body.copias_disponibles)
    return result


@router.delete("/{id_libro}", response_model=MessageResponse)
def delete_libro(id_libro: int, user=Depends(require_permission("Libro", "delete"))):
    with get_session() as session:
