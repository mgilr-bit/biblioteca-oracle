...
Nueva funcionalidad: cuando un préstamo se devuelve después de la fecha
esperada se genera una multa fija de Q35 en estado PENDIENTE. Una multa
pendiente bloquea la creación de nuevos préstamos para ese usuario hasta
que el bibliotecario la marque como PAGADA o CONDONADA.

Idempotente a propósito: el borrador `database/08_mejoras_recomendadas.sql`
ya traía una tabla `multas` parcial (sin columnas de auditoría/soft-delete
ni `dias_retraso`), marcada como "funcionalidad futura". Como algunos
entornos ya corrieron ese script:

* si `multas` NO existe -> se crea completa (caso instalación limpia / Docker).
* si `multas` YA existe  -> solo se agregan las columnas que falten para
  alinearla con models/multa.Multa + models/base.AuditMixin.

A partir de aquí, esta migración es la única fuente de verdad del esquema
de `multas` (ese bloque se quitó del .sql).
...
# Columnas de auditoría/soft-delete (AuditMixin) + dias_retraso. Son las que
# le faltan a la tabla si vino del borrador de database/08_*.sql.
_COLUMNAS_EXTRA = [
...
def downgrade() -> None:
    # Baja simétrica solo para el caso de creación limpia; si la tabla venía
    # del borrador de database/08_*.sql, esta baja igual la elimina.
