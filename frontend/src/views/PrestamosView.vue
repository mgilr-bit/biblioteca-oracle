...
const filteredPrestamos = computed(() => {
  return allPrestamos.value.filter((p) => {
    if (filters.value.fechaPrestamo && extractDate(p.FECHA_PRESTAMO) !== filters.value.fechaPrestamo) {
      return false
    }
    if (
    ...
    if (filters.value.libro && !p.TITULO?.toLowerCase().includes(filters.value.libro.toLowerCase())) {
      return false
    }
    return true
  })
  ...
  try {
    if (tab === 'todos') {
      ...
      if (auth.isBibliotecario) {
        ...
        allPrestamos.value = todos.filter((p) => p.ESTADO === 'ACTIVO' || p.ESTADO === 'VENCIDO')
      }
...
async function devolver(prestamo) {
  const ok = await ask(`¿Registrar la devolución de "${prestamo.TITULO}"?`, { title: 'Devolver libro' })
  if (!ok) return
...
<template>
  <div class="stack">
    <div class="page-header">
      ...
      <button v-if="auth.isBibliotecario" class="btn btn-primary" @click="openModal">
        + Nuevo préstamo
      </button>
    </div>
      ...
      <div class="card__body">
        <div class="search-bar">
          <div class="field" style="margin:0">
            ...
            <input id="fp" v-model="filters.fechaPrestamo" type="date" class="input" />
          </div>
          <div class="field" style="margin:0">
            <label for="fd">Fecha de devolución</label>
            <input id="fd" v-model="filters.fechaDevolucion" type="date" class="input" />
          </div>
          <div class="field" style="margin:0; grid-column: span 2">
            <label for="lb">Libro</label>
            <input id="lb" v-model="filters.libro" class="input" placeholder="Buscar por título…" />
          </div>
        </div>
    ...
    <div class="cluster" role="tablist">
      <button
        v-for="t in [['todos', 'Todos'], ['activos', 'Activos'], ['vencidos', 'Vencidos']]"
        :key="t[0]"
        class="app-nav__link"
    ...
    <div class="card">
      <div class="card__body table-wrap">
        <table class="data">
          <thead>
            <tr>
              <th>ID</th><th>Libro</th><th>Autor</th>
              <th v-if="auth.isBibliotecario">Usuario</th>
              <th>Préstamo</th><th>Devolución</th><th>Estado</th><th>Acciones</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="p in filteredPrestamos" :key="p.ID_PRESTAMO">
              ...
              <td>{{ p.TITULO }}</td>
              <td>{{ p.AUTOR || '-' }}</td>
              <td v-if="auth.isBibliotecario">{{ p.NOMBRE_USUARIO || '' }}</td>
              <td>{{ new Date(p.FECHA_PRESTAMO).toLocaleDateString() }}</td>
              <td>{{ new Date(p.FECHA_DEVOLUCION_ESPERADA).toLocaleDateString() }}</td>
              <td><span class="stamp" :class="badgeClass(p.ESTADO)">{{ p.ESTADO }}</span></td>
              <td>
                <button
                  v-if="p.ESTADO !== 'DEVUELTO' && auth.isBibliotecario"
            ...
            <tr v-if="!loading && !filteredPrestamos.length">
              <td :colspan="auth.isBibliotecario ? 8 : 7" class="cell-empty">No hay préstamos</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <AppModal v-if="showModal" title="Nuevo préstamo" @close="showModal = false">
      <form class="stack" @submit.prevent="savePrestamo">
        <div class="field">
          <label for="libroSelect">Libro *</label>
          <select id="libroSelect" v-model="form.id_libro" class="select" required>
            <option value="">Seleccione un libro</option>
            <option v-for="l in libroOptions" :key="l.ID_LIBRO" :value="l.ID_LIBRO">
              {{ l.TITULO }} — {{ l.AUTOR }} ({{ l.COPIAS_DISPONIBLES }} disponibles)
            </option>
          </select>
        </div>
        <div class="field">
          <label for="usuarioSelect">Usuario *</label>
          <select id="usuarioSelect" v-model="form.id_usuario" class="select" required>
            <option value="">Seleccione un usuario</option>
            <option v-for="u in usuarioOptions" :key="u.ID_USUARIO" :value="u.ID_USUARIO">
              {{ u.NOMBRE }} ({{ u.EMAIL }})
            </option>
          </select>
        </div>
        <div class="field" style="margin-bottom:0">
          ...
          <input id="dias" v-model="form.dias_prestamo" type="number" min="1" class="input" />
        </div>
      </form>
      <template #footer>
        <button class="btn btn-outline" @click="showModal = false">Cancelar</button>
        <button class="btn btn-primary" :disabled="saving" @click="savePrestamo">
